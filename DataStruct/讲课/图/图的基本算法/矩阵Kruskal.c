...
typedef struct      //边数组
{
    int pre,next,power;         //边的起点和终点下标以及边的权值
}road;
...
void assign(road* a,road* b)        //将b赋值给a
{
    (*a).next = (*b).next;
    (*a).power = (*b).power;
    (*a).pre = (*b).pre;
...
void quicksort(road *r,int low,int high)         //快速排序边集数组
{
    ...
    while(i<j)
    {
        while(i<j&&r[j].power>temp.power)   
            --j;
        assign(&r[i],&r[j]);
        while(i<j&&r[i].power<=temp.power)  
            ++i;
        assign(&r[j],&r[i]); 
    }
    assign(&r[i],&temp);
...
int getroot(int a,int *v)       //并查集的查找
{
    while (a!=v[a])
        a = v[a];
    return a;
